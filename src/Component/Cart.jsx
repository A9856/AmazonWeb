...
export default function Cart({ title, data }) {
  ...
  return (
    <>
      <h5 className='text-light text-center p-2 bg-dark'>{title}</h5>
      {
        cart.length ?
          <>
                      ...
                      return <tr key={item.id}>
                        ...
                        <td>
                          <div className='btn-group w-100'>
                            <button className={`${title !== "Cart" ? 'd-none' : ''} btn btn-dark`} onClick={() => updateRecord(item.id, "DEC")}><i className='fa fa-minus'></i></button>
                            <h4 className='text-center w-25'>{item.qty}</h4>
                            <button className={`${title !== "Cart" ? 'd-none' : ''} btn btn-dark`} onClick={() => updateRecord(item.id, "INC")}><i className='fa fa-plus'></i></button>
                          </div>
                        </td>
                        <td>&#8377;{item.total}</td>
                ...
                <table className='table table-bordered table-striped table-hover'>
                  <tbody>
                    ...
                    <tr className={title !== "Products in Cart" ? 'd-none' : ''}>
                      <th>Payment Mode</th>
                      <td>
                        <select name='mode' onChange={(e) => setMode(e.target.value)} className='form-select border-3 border-dark'>
                          <option value="COD">COD</option>
                          <option value="Net Banking" disabled>NetBanking/Card/UPI</option>
                        </select>
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td colSpan={2} className={title === "Cart" ? 'd-none' : ''}>
                        <button className='btn btn-dark w-100' onClick={placeOrder}>Place Order</button>
                      </td>
                    </tr>
                  </tbody>
                </table>
